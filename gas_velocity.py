...
def compute_gas_velocity():
    ...
    if par.plot_gas_quantities == 'Yes':
        ...
        vvmid  = vtheta3D[par.gas.ncol//2,:,:]/1e5 # nrad, nsec   # in km/s
        ...
        print('--------- c) plotting midplane vertical velocity (x,y) ----------')
        ...
        mynorm = matplotlib.colors.Normalize(vmin=vvmid.min(),vmax=vvmid.max())
        vvmid = np.transpose(vvmid)
        CF = ax.pcolormesh(X,Y,vvmid,cmap='nipy_spectral',norm=mynorm,rasterized=True)
