...
def exportfits():
    ...
    if par.polarized_scat == 'No':
        ...
        # file image.fits does not exist yet, we need to create it
        ...
        # if image.fits, read it instead of image.out (much faster!)
            print('--------- reading image.fits ----------')
            f2 = fits.open('image.fits')
            images = f2[0].data
            f2.close()
        
        # dust continuum RT calculations (implicitly)
        if nlam == 1:  
            im = images.reshape(im_ny,im_nx)

        # gas RT calculations (implicitly)
        if nlam > 1:   
